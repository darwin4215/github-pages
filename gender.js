...
// Obtener todos los héroes o filtrar por gender
app.get('/heroes', async (req, res) => {
  try {
    ...
    // Obtener el parámetro 'gender_name' de la solicitud (opcional)
    const { gender_name } = req.query;

    const filter = {};
    if (gender_name) {
      const gender = await genderCollection.findOne({ name: gender_name });
      if (gender) {
        filter.gender_id = gender.gender_id;
      } else {
