...
// Obtener todos los héroes o filtrar por alignment
app.get('/heroes', async (req, res) => {
  try {
    await client.connect();
    const database = client.db('heroes');
    const heroCollection = database.collection('hero_information');
    const alignmentCollection = database.collection('alignment');

    // Obtener el parámetro 'alignment_name' de la solicitud (opcional)
    const { alignment_name } = req.query;

    const filter = {};
    if (alignment_name) {
      const alignment = await alignmentCollection.findOne({ name: alignment_name });
      if (alignment) {
        filter.alignment_id = alignment.alignment_id;
      } else {
        return res.status(404).json({ message: "Alignment not found" });
      }
    }

    // Join heroes with publisher and alignment information
