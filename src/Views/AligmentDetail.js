...
const AlignmentDetail = () => {
    const { alignmentName } = useParams(); // Obtener el bando (alignment_name) desde la URL
    const [heroes, setHeroes] = useState([]); // Estado para los héroes
    const [error, setError] = useState(null); // Estado para manejar errores

    // Función para obtener y mostrar los datos de MongoDB
    const fetchHeroesByAlignment = async () => {
        try {
            const response = await fetch(`http://localhost:3003/heroes?alignment_name=${alignmentName}`);
            if (!response.ok) {
                throw new Error('');
            }
    ...
    const deleteHero = async (heroId) => {
        try {
            const response = await fetch(`http://localhost:3003/heroes/${heroId}`, {
                method: 'DELETE',
            });

            if (response.ok) {
                fetchHeroesByAlignment(); // Refrescar la lista de héroes después de eliminar
            } else {
    ...
    const updateHero = async (event, heroId) => {
        ...
        try {
            ...
            if (response.ok) {
                fetchHeroesByAlignment(); // Refrescar la lista de héroes después de actualizar
            } else {
    ...
    // Ejecutar la función fetchHeroesByAlignment cuando el componente se monte
    useEffect(() => {
        fetchHeroesByAlignment();
    }, [alignmentName]); // Actualizar cuando el alignmentName cambie

    return (
        <div>
            <h1>Héroes de bando: {alignmentName}</h1>
